...
public class MainTravelTimeService {
	...
	public Docket docket(ServletContext servletContext) {
		return new Docket(DocumentationType.SWAGGER_2).apiInfo(apiInfo()).select().apis(RequestHandlerSelectors.any())
				.paths(PathSelectors.regex("/TravelTimeService.*")).build()
				.securitySchemes(Arrays.asList(new ApiKey("api_key", "Authorization", "header")))
				.securityContexts(Arrays.asList(SecurityContext.builder()
						.securityReferences(Arrays.asList(new SecurityReference("api_key", new AuthorizationScope[0])))
						.forPaths(PathSelectors.regex("/*.*")).build()))
				.host("dlr-integration.minglabs.com").pathProvider(new RelativePathProvider(servletContext) {
					@Override
	...
	private ApiInfo apiInfo() {
		return new ApiInfoBuilder().title("ExCELL TravelTimeService").description(
				...
				.version("1.0").contact(new Contact("TU Dresden - Chair of Traffic Control and Process Automatisation",
						"http://tu-dresden.de/vlp", "sebastian.pape@tu-dresden.de"))
				.build();
	}
	...
	public class ConfigurerAdapter implements WebMvcConfigurer {
		...
		public void addViewControllers(ViewControllerRegistry registry) {
			...
			registry.addViewController(swaggerBaseUrl + "/swagger-resources/configuration/ui")
					.setViewName("forward:/swagger-resources/configuration/ui");
			registry.addViewController(swaggerBaseUrl + "/swagger-resources/configuration/security")
